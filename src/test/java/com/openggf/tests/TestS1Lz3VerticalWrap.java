 ...
 * Sonic and nearby objects must remain visible — they must not vanish.
 * <p>
...
public class TestS1Lz3VerticalWrap {
     ...
     * consistent — Sonic must remain visible on screen.
     */
    @Test
    public void sonicRemainsVisibleThroughUpwardWrap() {
        ...
        assertTrue("Vertical wrap should have occurred during the test", wrapOccurred);
        assertFalse("Sonic should remain visible after camera vertical wrap (vanished at frame "
                + vanishFrame + ")", sonicVanished);
    }
    ...
    public void sonicRemainsVisibleThroughDownwardWrap() {
        ...
        assertTrue("Vertical wrap should have occurred during the test", wrapOccurred);
        assertFalse("Sonic should remain visible after camera downward wrap (vanished at frame "
                + vanishFrame + ")", sonicVanished);
    }
     ...
     * — the object should be on-screen.
     * <p>
    ...
    public void cameraBoundsContainsWrappedY() {
        ...
        assertTrue("Object at Y=3946 should be on-screen with camera at Y=1788 (wrap range 2048)",
                bounds.contains(1500, 3946));

        // Object at Y=1900 (directly in the camera range)
        assertTrue("Object at Y=1900 should be on-screen with camera at Y=1788",
                bounds.contains(1500, 1900));

        // Object at Y=100 (far from camera in modular space: distance 360)
        // (100-1788) % 2048 + 2048 = 360. 360 > 224, so off screen.
        assertFalse("Object at Y=100 should be off-screen with camera at Y=1788",
                bounds.contains(1500, 100));
        ...
        assertFalse("Without wrapping, Y=3946 should be off-screen (linear check)",
                linearBounds.contains(1500, 3946));
    }
    ...
    public void renderYAdjustmentMatchesVdpWrap() {
        ...
        assertTrue("Adjusted screen Y should be on-screen (0-224), got " + screenY,
                screenY >= 0 && screenY <= 224);
        assertEquals("Modular screen Y should be 110 (3946-1788=2158, 2158%2048=110)",
                110, screenY);

        // Object at Y=100, camera at Y=1788 — should remain off-screen
        ...
        assertTrue("Object at Y=100 should stay off-screen, screenY=" + screenFar,
                screenFar > 224 || screenFar < -64);

        // Object already near camera (Y=1900, camera=1788) — no significant change
        int adjustedNear = adjustYForWrap(1900, cameraY, wrapRange);
        assertEquals("Object at Y=1900 should not change", 1900, adjustedNear);
    }
