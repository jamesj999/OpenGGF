...
public class TestArzRunRight {
    ...
    /**
     * Simulate the spring-loop scenario: run left to hit spring, then track
     * everything as Sonic traverses the loop rightward.
     *
     * This is the scenario where the user reports Sonic getting stuck at X~2600.
     * The test tracks collision path changes to determine if a plane switcher
     * incorrectly puts Sonic on the secondary path during the loop.
     */
    @Test
    public void debugSpringLoopWithPathTracking() {
        Camera cam = Camera.getInstance();

        System.out.printf("INITIAL: X=%d Y=%d path=0x%02X/0x%02X%n",
        ...
        for (int f = 0; f < 600; f++) {
            ...
            if (sprite.getGSpeed() > 0x200) {
                System.out.printf("SPRING HIT at f%d: X=%d Y=%d G=%d path=0x%02X/0x%02X%n",
        ...
        assertTrue("Should hit spring while running left", hitSpring);

        // Phase 2: After spring, run with no input and track everything
        ...
        for (int f = 0; f < 600; f++) {
            ...
            // Log near the problem area or on anomalies
            boolean nearPlaneSwitcher = (x >= 2540 && x <= 2620);
            boolean inLoopYRange = (y >= 448 && y <= 704);
            boolean anomaly = (g == 0 && prevG != 0 && !sprite.getAir()) || (dx < -3);

            if (f < 20 || f % 30 == 0 || nearPlaneSwitcher || anomaly || (inLoopYRange && x >= 2500)) {
                System.out.printf("f%d: X=%d Y=%d G=%d Air=%b Ang=0x%02X Mode=%s path=0x%02X/0x%02X dx=%d%n",
                        f, x, y, g, sprite.getAir(), angle,
        ...
        if (stuck) {
            System.out.printf("%n=== DIAGNOSIS ===%n");
            if (sprite.getTopSolidBit() == 0x0E || sprite.getLrbSolidBit() == 0x0F) {
                System.out.println("Sonic is on SECONDARY collision path.");
                System.out.println("A plane switcher likely fired during loop traversal,");
                System.out.println("switching Sonic to secondary where chunk 204 has a wall.");
                System.out.println("Check plane switcher at (2576, 576, subtype=0x0A):");
                System.out.println("  It fires when X crosses 2576 while Y is in [448, 704).");
            } else {
                System.out.println("Sonic is on PRIMARY collision path but still stuck.");
                System.out.println("This suggests a physics issue, not a plane switcher issue.");
            }
        }

        assertFalse("Sonic should not get stuck during loop traversal. " +
                        "Stuck at frame " + stuckFrame + ", X=" + sprite.getX() +
                        ", path=0x" + String.format("%02X", sprite.getTopSolidBit()) +
                        "/0x" + String.format("%02X", sprite.getLrbSolidBit()),
                stuck);
    }
