...
public class SwScrlDez implements ZoneScrollHandler {
    ...
    private void fillScrollBuffer(int[] horizScrollBuf, int cameraX) {
        ...
        short bgScroll = negWord(tempArray[segIdx]);
        ...
        for (int line = 0; line < VISIBLE_LINES; line++) {
            ...
            linesInSegment--;
            if (linesInSegment == 0 && line < VISIBLE_LINES - 1) {
                // Fetch new segment
                if (segIdx < numSegments) {
                    linesInSegment = heights[segIdx] & 0xFF;
                    bgScroll = negWord(tempArray[segIdx]);
                    packed = packScrollWords(fgScroll, bgScroll);
                    trackOffset(fgScroll, bgScroll);
                    segIdx++;
                }
            }
