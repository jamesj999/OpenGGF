...
CAGE_HOOKS_REGISTERED = false

function register_cage_hooks()
...
CAGE_INIT_PTR = 0x00033836       -- Obj_CNZWireCage (label at sonic3k.asm:69813)
CAGE_FRAME_PTR = 0x0003385E      -- loc_3385E (per-frame entry at sonic3k.asm:69829)

function emit_cage_state_per_frame()
    if not aux_file then return end
    local vfc = mainmemory.read_u16_be(ADDR_FRAMECOUNT)
    for slot = 0, OBJ_TOTAL_SLOTS - 1 do
        local addr = OBJ_TABLE_START + (slot * OBJ_SLOT_SIZE)
