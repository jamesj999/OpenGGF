...
- [x] AIZ1 mid-act transition: weird snapping/teleportation occurs after the transition once the player goes past the first set of spikes. **(Fixed: `updateAiz2SonicResize1`/`KnuxResize1` unconditionally skipped the miniboss path and set `Camera_min_X_pos=$F50` at cameraX>=$2E0, snapping the player forward. ROM gates this on `Apparent_zone_and_act==AIZ2`; added `enteredAsAct2` flag to distinguish fire-transition arrival from direct entry; covered by `TestSonic3kAIZEvents`)**
